...
{
  wattron(game_win, COLOR_PAIR(white_pair));
  for(int y = 0; y < player_one->height; y++) {mvwaddstr(game_win, player_one->y + y, player_one->x, "|");}
  for(int y = 0; y < player_two->height; y++) {mvwaddstr(game_win, player_two->y + y, player_two->x, "|");}
  wattroff(game_win, COLOR_PAIR(white_pair));
}
...
void exit_game(WINDOW *game_win, WINDOW *score_win, ball_t *ball, player_t *player_one, player_t *player_two)
{
  free(player_one);
  player_one = NULL;
